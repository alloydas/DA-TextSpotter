...
__all__ = [
    ...
    "LambdaBottleneckBlock"
    "build_lambdaresnet_backbone",
]
...
class LambdaBottleneckBlock(CNNBlockBase):
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.conv2 = nn.ModuleList([LambdaConv(bottleneck_channels, bottleneck_channels)])
        if stride != 1 or bottleneck_channels != self.expansion * bottleneck_channels:
            self.conv2.append(nn.AvgPool2d(kernel_size=(3, 3), stride=stride_3x3, padding=1 * dilation))
        self.conv2.append(nn.BatchNorm2d(bottleneck_channels))
...
def build_lambdaresnet_backbone(cfg, input_shape):
    """
    ...
    for idx, stage_idx in enumerate(range(2, 6)):
        ...
        if depth in [18, 34]:
            ...
            if deform_on_per_stage[idx]:
                ...
                stage_kargs["block_class"] = LambdaBottleneckBlock
...
@BACKBONE_REGISTRY.register()
def build_lambda1resnet_backbone(cfg, input_shape):
    """
    Create a ResNet instance from config.
