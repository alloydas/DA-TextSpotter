...
class FocalTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        if self.focal_level > 1 and self.pool_method != "none": 
            ...
            for k in range(self.focal_level-1):     
                window_size_glo = math.floor(self.window_size_glo / (2 ** k))
                pooled_h = math.ceil(Hp / self.window_size) * (2 ** k)
                pooled_w = math.ceil(Wp / self.window_size) * (2 ** k)
...
class PatchMerging(nn.Module):
    ...
    def __init__(self,  in_chans=3, norm_layer=nn.LayerNorm):
        super().__init__()
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim,  depth, num_heads, window_size, expand_size, expand_layer="all",
...
class FocalTransformer(Backbone):
    ...
    def train(self, mode=True):
        ...
        super(FocalTransformer, self).train(mode)
        self._freeze_stages()
...
def build_focal_tiny_backbone(cfg, input_shape):
    """
    Create a SwinT instance from config.
    Returns:
        VoVNet: a :class:`VoVNet` instance.
    """
    out_features = cfg.MODEL.SWINTV1.OUT_FEATURES

    return FocalTransformer(
        embed_dim=96, depths=[2,2,6,2], drop_path_rate=0.2, 
        ...
        out_features=cfg.MODEL.SWINTV2.OUT_FEATURES
    )
